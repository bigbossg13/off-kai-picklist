...
function rankBadgeStyle(rank: number, drafted: boolean) {
  if (drafted) return { bg: '#1e1e35', text: '#374151' };
...
export function TeamCard({ team, onRemove, onToggleDrafted, onClick }: TeamCardProps) {
  const {
    attributes, listeners, setNodeRef, transform, transition, isDragging,
  } = useSortable({ id: team.teamNumber, disabled: team.loading || team.drafted });
  ...
  const badge = rankBadgeStyle(team.rank, team.drafted);
  ...
  return (
    <div
      ...
      style={{
        ...style,
        background: team.drafted ? '#111121' : isDragging ? '#1e1e35' : '#16162a',
        border: `1px solid ${team.drafted ? '#1a1a2e' : isDragging ? '#8b5cf6' : '#252542'}`,
        borderRadius: '12px',
        opacity: team.drafted ? 0.5 : isDragging ? 0.7 : 1,
    ...
    >
      ...
      <div
        {...attributes}
        {...listeners}
        style={{ color: team.drafted ? '#1f2937' : '#374151', cursor: (team.loading || team.drafted) ? 'default' : 'grab' }}
      ...
      >
        {team.drafted ? '–' : team.rank}
      </div>

      {/* Team info */}
      <div
        className="flex-1 min-w-0 cursor-pointer"
        onClick={() => !team.loading && !team.error && !team.drafted && onClick(team)}
      >
        {team.loading ? (
          ...
          <div className="flex items-center gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span
                  style={{ color: team.drafted ? '#374151' : '#8b5cf6' }}
                ...
                {team.drafted && (
                  <span
                    style={{ background: '#1e1e35', color: '#4b5563', borderRadius: '4px' }}
                    className="text-xs px-1.5 py-0.5"
                  >
                    PICKED
                  </span>
                )}
                {!team.drafted && team.state && (
                  <span
              ...
              <p
                ...
                style={{
                  color: team.drafted ? '#374151' : '#e2e8f0',
                  textDecoration: team.drafted ? 'line-through' : 'none',
                }}
              >
                {team.name}
              </p>
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <div className="text-right">
            <div
              ...
              style={{ color: team.drafted ? '#374151' : epaColor(team.epaTotal) }}
            >
      ...
      {!team.loading && !team.error && (
        <button
          onClick={() => onToggleDrafted(team.teamNumber)}
          title={team.drafted ? 'Undo pick' : 'Mark as picked'}
          style={{
            color: team.drafted ? '#16a34a' : '#374151',
            background: 'none',
            border: 'none',
            cursor: 'pointer',
            padding: '2px',
